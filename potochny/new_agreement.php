...
require_once('../include/dbconnect.php'); //Подключаем файл с параметрами подключения к БД
 ...
 //Присваиваем переменным нулевые значения, чтобы выводимая пустая форма не выдавала ошибки
  ...
  if (!$output_form){
    ...
    "VALUES ('$rnz', '$data_reestr', '$vid_rabot', '$zakazchik', '$zadacha', '$manager', '$gorod', 
	'$adres', '$summa_rabot', '$tip_dzerela', '$dzerelo', '$manager_sdelki', '$etc')";
    ...
    $table = "<table>";
       ...
       $table .= "<th>".'Дата договору'."</th>";
       $table .= "<th>".'Вид робіт'."</th>";
       $table .= "<th>".'Замовник'."</th>";
       $table .= "<th>".'Предмет договору'."</th>";
       $table .= "<th>".'Менеджер'."</th>";
	   $table .= "<th>".'Місто робіт'."</th>";
	   $table .= "<th>".'Адреса робіт'."</th>";
	   $table .= "<th>".'Тип джерела інформації'."</th>";
	   $table .= "<th>".'Джерело інформації'."</th>";
	   $table .= "<th>".'Менеджер угоди'."</th>";
...
<form id="Form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
	<div>
		...
		<div  class="wrapper">
			...
			<div class="bg"><input type="date" name="data_reestr" class="input" value="<?php echo $data_reestr; ?>" ></div>
		</div>
		<div  class="wrapper">
			<span>Вид робіт: *</span>
			<div class="bg"><select name="vid_rabot" class="select">
		...
		<div  class="wrapper">
			...
			<div class="bg"><select name="zakazchik" class="select">
		...
		<div  class="textarea_box">
			...
			<div class="bg"><textarea name="zadacha" cols="52" rows="3"  value="<?php echo $zadacha; ?>"></textarea></div>
		</div>
		<div  class="wrapper">
			<span>Менеджер: *</span>
		...
		<div  class="wrapper">
			...
			<div class="bg"><select id="oblast" name="oblast" class="select" >
		...
		<div  class="wrapper">
			<span>&nbsp;</span>
			<div class="bg"><select id="region" name="region" class="select" >
		...
		<div  class="wrapper">
			<span>&nbsp;</span>
			<div class="bg"><select id="gorod" name="gorod" class="select" >
		...
		<div  class="textarea_box">
			...
			<div class="bg"><textarea name="adres" cols="52" rows="2"  value="<?php echo $adres; ?>"></textarea></div>
		</div>		
		<div  class="wrapper">
			<span>Вартість робіт за договором: *</span>
			<div class="bg"><input type="text" name="summa_rabot" class="input" value="<?php echo $summa_rabot; ?>"></div>
		</div>
		<div  class="wrapper">
			<span>Тип джерела інформації:</span>
			<div class="bg"><select name="tip_dzerela" class="select">
		...
		<div  class="wrapper">
			...
			<div class="bg"><input type="text" name="dzerelo" class="input" value="<?php echo $dzerelo; ?>"></div>
		</div>
		<div  class="wrapper">
			<span>Менеджер угоди:</span>
			<div class="bg"><select name="manager_sdelki" class="select">
				<option value="0">Оберіть менеджера</option>
					<?php
					$dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME)
					or die('Error connecting to MySQL server.');

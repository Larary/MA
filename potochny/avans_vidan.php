...
require_once('../include/dbconnect.php'); //Подключаем файл с параметрами подключения к БД
 ...
 //Присваиваем переменным нулевые значения, чтобы выводимая пустая форма не выдавала ошибки
	...
	/* $istochnik_oplat = trim($_POST['istochnik_oplat']);
	if (empty($istochnik_oplat)) {
  ...
  if (!$output_form){
    ...
    $query = "INSERT INTO r_avans_vidan (data, istochnik_oplat, manager, suma, etc) " .
    "VALUES ('$data', '$istochnik_oplat', '$manager', '$suma', '$etc')";
    ...
    $table = "<table>";
       ...
       $table .= "<th>".'Дата видачі авансу'."</th>";
       $table .= "<th>".'Джерело оплати'."</th>";
       $table .= "<th>".'Менеджер'."</th>";
       $table .= "<th>".'Сума авансу'."</th>";
	...
	<form id="Form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
		<div>
			...
			<div class="wrapper">
				...
				<div class="bg"><input type="text" name="suma" class="input" value="<?php echo $suma; ?>"></div>
			</div>

...
if (isset($_POST['submit'])){ //Проверка, была ли уже отправка данных
    ...
    if (!isset($_POST['number'])){ //Если чекбоксы на удаление значений не нажимались
        ...
        if(!empty($_POST['data_planovo'])) {
            ...
            $result4 = mysqli_query($dbc, $query4) or die('Error querying database.');
        }
        if(!empty($_POST['data_vykon'])) {
            ...
            $result5 = mysqli_query($dbc, $query5) or die('Error querying database.');
        }
        if(!empty($_POST['nalogi'])) {
            ...
            $result6 = mysqli_query($dbc, $query6) or die('Error querying database.');
        } 
        ...
        if(!empty($_POST['geodezia'])) {
            ...
            $result7 = mysqli_query($dbc, $query7) or die('Error querying database.');
        } 
        ...
        if(!empty($_POST['agentskie'])) {
            ...
            $result8 = mysqli_query($dbc, $query8) or die('Error querying database.');
        } 
        ...
        if(!empty($_POST['bonus'])) {
            ...
            $result9 = mysqli_query($dbc, $query9) or die('Error querying database.');
        } 
        ...
        if($change){
            ...
            $result10 = mysqli_query($dbc, $query10) or die('Error querying database.');
        }
        if(!empty($_POST['etc'])) {
            $etc = trim($_POST['etc']);
            $etc = mysqli_real_escape_string($dbc, $etc);
            $query11 = "UPDATE r_reestr2 SET etc='$etc' WHERE rnz='$rnz'";
            $result11 = mysqli_query($dbc, $query11) or die('Error querying database.');
        }
    }
    ...
    $result = mysqli_query($dbc, $sql) or die(mysql_error());
    $table = "<table><tr><th></th><th>Номер договору</th><th>Планові витрати</th><th>Премія</th><th>Стадія робіт</th>

...
$title='Звіт про обсяги діяльності в розрізі менеджерів';
...
<div id="content">
    <h3>Звітність</h3>
<?php 
    $h2 = '<h2>Звіт про обсяги діяльності в розрізі менеджерів</h2>';
    echo $h2;
...
if (!$output_form){
    ...
    $query1 = "SELECT SUM(summa_rabot), SUM(zatraty_plan), SUM(premia), SUM(nalogi), SUM(geodezia), SUM(agentskie), SUM(bonus), SUM(pribil_plan) 
		FROM r_reestr1 INNER JOIN r_reestr2 ON r_reestr1.rnz=r_reestr2.rnz WHERE manager='$manager' AND data_reestr BETWEEN '$date_begin' AND '$date_end'";
	...
	WHERE r_reestr1.manager='$manager' AND data_reestr BETWEEN '$date_begin' AND '$date_end'";
	...
	WHERE manager='$manager' AND data_reestr BETWEEN '$date_begin' AND '$date_end'";
    $result3 = mysqli_query($dbc, $query3) or die('Error querying database3.');
    while ($row = mysqli_fetch_array($result3)){
	$oplata_fakt = $row['SUM(oplata_fakt)'];
    }
	
    $query4 = "SELECT fio_kratko FROM manager WHERE pp='$manager'";
    $result4 = mysqli_query($dbc, $query4) or die('Error querying database4.');
    while ($row = mysqli_fetch_array($result4)){
	$fio_manager = $row['fio_kratko'];
    }
    ...
    $h4 = "<h4>Звіт за менеджером ".$fio_manager." за період з ".$date_begin." по ".$date_end."</h4>";
    echo $h4;
?>
    <div id="report">	
<?php

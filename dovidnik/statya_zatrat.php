...
if (!isset($_POST['submit'])) { //Если ОК еще не нажимали и данные не отправляли
 //Присваиваем переменным нулевые значения, чтобы выводимая пустая форма не выдавала ошибки
    ...
    if (!empty($_POST['vid_zatrat'])) {
        $vid_zatrat = $_POST['vid_zatrat'];
    }
    else {
        echo "<p class='errortext'>Не вибрано вид витрат.</p><br />";
        $vid_zatrat = null;
	$output_form = true;
    }
    $etc = trim($_POST['etc']);	    
...
if (!$output_form){
    ...
    $query = "INSERT INTO statya_zatrat (statya_zatrat, vid_zatrat, etc) VALUES ('$statya_zatrat', '$vid_zatrat', '$etc')";
    ...
    $table = "<table><tr><th>Стаття витрат</th><th>Вид витрат</th><th>Примітки</th</tr>>";
    ...
    <form id="Form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <div>
            <div  class="wrapper">
                ...
                <div class="bg">
                    <input type="text" name="statya_zatrat" class="input" value="<?php echo $statya_zatrat; ?>"/>
                </div>
            </div>
            <div  class="wrapper">
                <span>Вид витрат: *</span>
                <div class="radio">
                    Проектні&nbsp;<input type="radio" name="vid_zatrat" value="проект" 
                    <?php 
                    if($vid_zatrat=="проект"){
                        echo 'checked';
                    }?>>&nbsp;&nbsp; 
